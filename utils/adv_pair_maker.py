...
def main():
    ...
    for i in range(args.class_num):
        for k in range(args.class_num):
            for x, y in itertools.product(source_path[i], target_path[k]):
                if i == k:
                    G2_pairs.append([x, y, 0])   #G2ペアにG1のラベル
                else:
                    G4_pairs.append([x, y, 2])   #G4ペアにG3のラベル
    ...
    df2.to_csv(os.path.join(args.csv_path, "adv_G2.csv"))
    df4.to_csv(os.path.join(args.csv_path, "adv_G4.csv"))
